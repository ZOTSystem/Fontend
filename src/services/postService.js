...
export const getPostBySubjectService = async (subjectId) => await request.get(`${END_POINTS.GET_BY_SUBJECT}?subjectId=${subjectId}`);

export const getPostByStatusService = async (status) => await request.get(`${END_POINTS.GET_BY_STATUS}?status=${status}`);

export const getPostBySubjectAndStatusService = async (subjectId, status) => await request.get(`${END_POINTS.GET_BY_SUBJECT_STATUS}?subjectId=${subjectId}&status=${status}`)
